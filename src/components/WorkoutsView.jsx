  ...
  return (
    <div className="px-4 pt-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold tracking-tight">Meine Workouts</h1>
        <button
          onClick={onCreate}
          className="text-orange-400 text-sm font-medium active:opacity-70"
        >
          + Neu
        </button>
      </div>
      ...
      {workouts.length === 0 && (
        <div className="text-center py-20 text-gray-500">
          ...
          <p className="text-lg font-medium text-gray-400 mb-1">Noch kein Workout</p>
...
function WorkoutCard({ workout, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl p-4 active:scale-[0.98] transition-transform"
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <span className={`text-xs font-bold uppercase tracking-wide ${typeColor} flex-shrink-0`}>
      ...
      {workout.result?.score && (
        <div className="flex items-center gap-1.5 mt-2">
          ...
          {workout.result.rxd && (
            <span className="text-xs bg-green-500/20 text-green-300 px-2 py-0.5 rounded-full font-medium">RX</span>
          )}
        </div>
      )}
    </button>
